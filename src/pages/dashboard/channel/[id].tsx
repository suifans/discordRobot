...
const Discord = () =>{
        ...
        if (router.isReady) {
            ...
            const query = async () =>{
                if(user_id!==""){
                    ...
                    const getAllRules = await client.callApi('v1/Test/GetDcUserTestRulesNumber', {
                        guild_id: `${guild_id}`
                    });

                    console.log(getAllRules.res)
                    const rulesList =getAllRules.res ? JSON.parse(getAllRules.res.rules) : []
                    //
    ...
    const Apply = (activeOption) =>{
        if(activeOption!==""){
            router.push(`/dashboard/channel/add1/${discordList[selectDiscordList].id}/${activeOption}`)
        }
            ...
            <div className="pt-10 flex flex-wrap gap-4 items-center">
                <div className={setUpRolesList[0][0].name==""?"hidden":"flex flex-wrap gap-4 items-center"}>
                    {setUpRolesList.map(item=>(
                        <div key={item[0].id} className="px-2 flex justify-between items-center pl-5 py-1.5  rounded-lg bg-[#33353D] w-56 h-10">
                            <button  onClick={()=>Apply(item[0].id)}className="flex items-center">
                                ...
                                <span className="text-sm font-medium text-white truncate pl-2">{item[0].name}</span>
                            </button>
                ...
                <button onClick={selectRole} className=" text-white w-56 h-10 py-1.5 rounded-lg text-sm flex justify-center items-center bg-[#33353D]" >
                    <img className="w-3 h-3 mr-2" src='/add.svg' alt=""/>
                    Select role
                </button>
                ...
                <Dialog as="div" className="fixed z-40 inset-0 overflow-y-auto " onClose={()=>false}>
                    <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center shadow-2xl   sm:block sm:p-0">
                        <Transition.Child
                            ...
                            leaveTo="opacity-0"
                        >
                            <Dialog.Overlay className="fixed inset-0 bg-[#1E1F24] bg-opacity-90 transition-opacity" />
                        </Transition.Child>
                        ...
                        >
                            <div className="inline-block align-bottom p-0.5 rounded-lg    rounded-lg  text-left overflow-hidden shadow-xl transform transition-all sm:y-8 sm:align-middle   ">
                                <div className="bg-[#33353D] px-4 py-5 sm:px-6  rounded-md">
                                    <div className=" text-white font-semibold text-2xl pt-8 pb-5">
                                        Select Discord role
                                    </div>
...
const NFT = () =>{
    ...
    const Apply = (activeOption) =>{
        if(activeOption!==""){
            router.push(`/dashboard/channel/add2/${discordList[selectDiscordList].id}/${activeOption}`)
        }
            ...
            <div className="flex ">
                ...
                <div className="w-full  overflow-hidden">
                    ...
                    <div className="h-screen  overflow-y-auto bg-[#1E1F24] p-10">
                        ...
                        <Tab.Group>
                            <Tab.List className="ml-4  space-x-1  mt-10 mx-auto  flex justify-between border-b border-[#2B2C33]">
                                <div>
                                    {TypeList.map((items) => (
                                        <Tab
                                            key={items.type}
                                            className={({ selected }) =>
                                                classNames(
                                                    ...
                                                    selected
                                                        ? 'border-b-2 text-white  border-[#707CAC]'
                                                        : ' text-gray-500 ')}>
                                            {items.type}
                                        </Tab>
                                    ))}
